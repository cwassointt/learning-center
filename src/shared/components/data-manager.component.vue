  ...
  methods:
      {
        ...
        confirmDeleteSelected() {
          this.$confirm.require( {
            ...
            reflectClass: 'p-button-secondary p-button-outlined',
            rejectLabel:  'Cancel',
            acceptLabel:  'Delete',
        ...
        confirmDeleteItem(item) {
          this.$confirm.require ( {
            message:      `Are you sure you want to delete the ${this.title.singular} ${item.name}?`,
            header:       'Confirmation',
            icon:         'pi pi-exclamation-triangle',
            reflectClass: 'p-button-secondary p-button-outlined',
            rejectLabel:  'Cancel',
...
<h3>Manage {{ title.plural }}</h3>
  ...
  <pv-toolbar class="mb-4">
    <template #start>
      ...
      <pv-button :disabled="!selectedItems || !selectedItems.length" icocn="pi pi-trash" label="Delete Selected" severity="danger" @click="confirmDeleteSelected" />
  ...
  <pv-data-table
    ...
    <pv-column v-if="dynamic" v-ford="column in columns" :key="column.field"
               :field="column.field"
